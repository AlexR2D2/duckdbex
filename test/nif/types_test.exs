defmodule Duckdbex.Nif.QueryTest do
  ...
  test "DATE", %{conn: conn} do
    ...
    assert {:ok, _} = NIF.query(conn, "INSERT INTO dates VALUES ('2000-01-01'), ('2022-01-31'), ('1999-12-31');")

    assert {:ok, r} = NIF.query(
      conn,
  ...
  test "TIME", %{conn: conn} do
    ...
    assert {:ok, _} = NIF.query(conn, "INSERT INTO times VALUES ('00:00:00'), ('01:59:59.999'), ('12:00:00');")
  ...
  test "TIMESTAMP", %{conn: conn} do
    ...
    assert [[{{2022, 10, 20}, {23, 59, 59, 999000}}]] =
      NIF.fetch_all(r)
  end
  ...
  test "HUGEINT", %{conn: conn} do
    ...
    assert {:ok, _} = NIF.query(conn, "INSERT INTO hugeints VALUES (-10::hugeint), (5::hugeint), (10::hugeint), (98233720368547758080000::hugeint);")

    assert {:ok, r} = NIF.query(conn, "SELECT * FROM hugeints WHERE value = $1;", ["5"])
    assert [[{0, 5}]] = NIF.fetch_all(r)

    assert {:ok, r} = NIF.query(conn, "SELECT * FROM hugeints WHERE value = $1;", ["98233720368547758080000"])
    assert [[{5325, 4808176044395724800}]] = NIF.fetch_all(r)

    assert {:ok, r} = NIF.query(conn, "SELECT * FROM hugeints WHERE value = $1;", [-10])
    assert [[{-1, 18446744073709551606}]] = NIF.fetch_all(r)

    hugeint = Duckdbex.integer_to_hugeint(98233720368547758080000)
    assert {:ok, r} = NIF.query(conn, "SELECT * FROM hugeints WHERE value = $1", [{:hugeint, hugeint}])
    assert [[{5325, 4808176044395724800}]] = NIF.fetch_all(r)
  end

  test "DECIMAL", %{conn: conn} do
    assert {:ok, _} = NIF.query(conn, "CREATE TABLE decimals(value DECIMAL(5,3));")
    assert {:ok, _} = NIF.query(conn, "INSERT INTO decimals VALUES (3.14::decimal), (0.8), (-22.174::decimal), (10::decimal);")

    assert {:ok, r} = NIF.query(conn, "SELECT * FROM decimals WHERE value = $1", [{:decimal, {-22174, 5, 3}}])
    # {value, width, scale}
    ...
    assert {:ok, r} = NIF.query(conn, "SELECT * FROM decimals WHERE value = $1", [{:hugeint, hugeint}])
  ...
  test "REAL", %{conn: conn} do
    ...
    assert {:ok, r} = NIF.query(conn, "SELECT * FROM reals WHERE value > $1 AND value <= $2;", [-2.18, 0.81])
  ...
  test "UBIGINT", %{conn: conn} do
    ...
    assert {:ok, r} = NIF.query(conn, "SELECT * FROM ubigints WHERE value = $1", [9223372036854775890])
    assert [_] = NIF.fetch_all(r)
  end
  ...
  test "UTINYINT", %{conn: conn} do
    ...
    assert {:ok, r} = NIF.query(conn, "SELECT * FROM utinyints WHERE value = $1 OR value = $2;", [1, 255])
    assert [_] = NIF.fetch_all(r)
  end
  ...
  test "TINYINT", %{conn: conn} do
    ...
    assert {:ok, r} = NIF.query(conn, "SELECT * FROM tinyints WHERE value = $1 OR value = $2;", [1, -127])
    assert [_, _] = NIF.fetch_all(r)
  end

  test "BLOB", %{conn: conn} do
    assert {:ok, _} = NIF.query(conn, "CREATE TABLE blobs(value BLOB);")
    assert {:ok, _} = NIF.query(conn, "INSERT INTO blobs VALUES ('hello!'::BLOB), ('world'::BLOB);")
  ...
  test "INTERVAL", %{conn: conn} do
    ...
    assert {:ok, _} = NIF.query(conn, "INSERT INTO intervals VALUES (INTERVAL 1 HOUR), (INTERVAL 1 DAY), (INTERVAL 2 MONTHS);")
  ...
  test "UUID", %{conn: conn} do
    ...
    """)
    assert {:ok, r} = NIF.query(conn, "SELECT * FROM uuids WHERE value = $1",   ["5e740554-23ad-11ed-861d-0242ac120002"])
  ...
  test "BOOLEAN", %{conn: conn} do
    ...
    assert {:ok, r} = NIF.query(conn, "SELECT * FROM booleans WHERE value = $1 OR value = $2", [true, false])
    assert [_, _] = NIF.fetch_all(r)
  ...
  test "BIGINT", %{conn: conn} do
    ...
    assert {:ok, _} = NIF.query(conn, "INSERT INTO bigints VALUES (-9223372036854775808), (9223372036854775807);")
    assert {:ok, r} = NIF.query(conn, "SELECT * FROM bigints WHERE value = $1 OR value = $2;", [-9223372036854775808, 0])
    assert [_] = NIF.fetch_all(r)
  end
  ...
  test "INTEGER", %{conn: conn} do
    ...
    assert {:ok, r} = NIF.query(conn, "SELECT * FROM integers WHERE value = $1 OR value = $2;", [0, -245])
    assert [_, _] = NIF.fetch_all(r)
  end
  ...
  test "DOUBLE", %{conn: conn} do
    ...
    assert {:ok, r} = NIF.query(conn, "SELECT * FROM doubles WHERE value = $1 OR value = $2;", [0.8, -2.17])
    assert [_, _] = NIF.fetch_all(r)
  end
  ...
  test "ENUM", %{conn: conn} do
    ...
    assert {:ok, _} = NIF.query(conn, "INSERT INTO person VALUES ('Pedro','happy'), ('Mark', NULL), ('Pagliacci', 'sad'), ('Mr. Mackey', 'ok');")
  ...
  test "LIST", %{conn: conn} do
    ...
    assert {:ok, _} = NIF.query(conn, "CREATE TABLE list_table (int_list INT[], varchar_list VARCHAR[]);")
    assert {:ok, _} = NIF.query(conn, "INSERT INTO list_table VALUES ([1, 2], ['one', 'two']), ([4, 5], ['three', NULL]), ([6, 7], NULL);")
  ...
  test "MAP", %{conn: conn} do
    ...
    assert {:ok, r} = NIF.query(conn, "select map([['a', 'b'], ['c', 'd']], [[1.1, 2.2], [3.3, 4.4]]);")
    assert [[%{["a", "b"] => [{11, 2, 1}, {22, 2, 1}], ["c", "d"] => [{33, 2, 1}, {44, 2, 1}]}]] = NIF.fetch_all(r)
    ...
    assert {:ok, _} = NIF.query(conn, "INSERT INTO map_table VALUES (map([1, 2], [2.98, 3.14])), (map([3, 4], [9.8, 1.6]));")

    assert {:ok, r} = NIF.query(conn, "SELECT * FROM map_table")
    assert [[%{1 => 2.98, 2 => 3.14}], [%{3 => 9.8, 4 => 1.6}]] = NIF.fetch_all(r)

    assert {:ok, r} = NIF.query(conn, "SELECT * FROM map_table WHERE map_col = $1", [%{1 => 2.98, 2 => 3.14}])
  ...
  test "STRUCT", %{conn: conn} do
    ...
    assert {:ok, r} = NIF.query(conn, "SELECT {'yes': 'duck', 'maybe': 'goose', 'huh': NULL, 'no': 'heron'};")
    assert [[%{"huh" => nil, "maybe" => "goose", "no" => "heron", "yes" => "duck"}]] = NIF.fetch_all(r)
    ...
    assert  [[%{
      "aliens" => nil,
      "amphibians" => %{"huh" => "dragon", "maybe" => "salamander", "no" => "toad", "yes" => "frog"},
  ...
  test "UNION", %{conn: conn} do
    ...
    assert {:ok, _} = NIF.query(conn, "INSERT INTO tbl1 values (1) , ('two') , (union_value(str := 'three'));")
    assert {:ok, r} = NIF.query(conn, "SELECT u from tbl1;")
